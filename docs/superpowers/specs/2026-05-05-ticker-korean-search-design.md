...
- 서버사이드 ticker 형태/존재 검증(별도 스펙). v1은 프론트엔드 가드(4.4)로 한글/이상 입력의 백엔드 누수를 차단하는 데 집중
...
- **검색어(query)와 확정 티커(committed ticker) 분리**:
  - 사용자가 키보드로 입력하는 동안의 raw 텍스트는 컴포넌트 내부 상태(`query`)로만 보관
  - 부모로 전달하는 `value`/`onChange`는 "확정된 티커"만 의미
- **확정 경로**:
  1. 드롭다운에서 결과 선택 → `onChange(ticker)` (항상 영문 대문자 티커)
  2. 자유 입력 후 Enter/blur → 입력값이 [4.4 검증 규칙](#44-입력-검증과-제출-가드)을 통과하면 `onChange(ticker.toUpperCase())`, 통과하지 못하면 확정하지 않고 인라인 안내(예: "한글 검색은 목록에서 선택해주세요")
- 빈 입력으로 비울 때는 `onChange("")` 허용

### 4.4 입력 검증과 제출 가드

자유 입력 경로(시드 미존재 영문 티커 허용)는 보존하되, 한글/별칭이 미확정 상태로 백엔드까지 흘러가는 것을 막는다.

- **확정 가능 조건**(아래 중 하나):
  1. 시드 결과를 드롭다운에서 선택했다(매치된 티커 그대로 사용)
  2. 시드에 정확히 매치되는 결과가 단 하나이고, 입력값이 그 티커 또는 별칭과 일치(공백/대소문자 무시)한다 → 그 티커로 확정
  3. 입력값이 영문 티커 패턴 정규식 `^[A-Z][A-Z0-9.\-]{0,15}$`을 만족한다(대문자화 후 검사). `BRK.B`, `BF-B`, `MSTR` 등 허용
- **확정 불가**(즉, 폼 제출 차단):
  - 한글이 한 글자라도 포함되어 있고 시드 매치 선택이 없을 때
  - 영문이지만 패턴 미충족(공백/특수문자/너무 긺)일 때
- **컴포넌트 동작**:
  - 확정 불가 상태에서 부모의 `value`는 직전에 확정된 값이 유지된다(혹은 `""`)
  - 컴포넌트는 `aria-invalid` + 빨간 테두리 + 인라인 메시지 노출
  - 폼 제출 버튼은 `value`가 비어 있거나 컴포넌트가 invalid면 disabled

### 4.5 대문자화

- 영문 자유 입력은 컴포넌트 내부에서 `onChange` 직전에 `toUpperCase()` 적용 (현재 RunForm/HoldingForm이 부모에서 하던 동작을 컴포넌트 안으로 이동).
- 한글 입력은 검색어(`query`) 안에서만 그대로 유지되며, 확정 단계까지 도달하지 못하므로 부모/백엔드로 전달되지 않는다.

### 4.6 의존성 / 접근성

- 기존 `@radix-ui/react-popover`만 사용. `cmdk` 등 추가 라이브러리 도입 없음.
- ARIA: `role="combobox"`, `aria-expanded`, `aria-controls`, `aria-activedescendant`, `aria-invalid`, 항목에 `role="option"`.
...
- 변경:
  ...
  - UI: `<TickerCombobox>` 1개 + **확정된 티커**(4.4 가드 통과)만 칩으로 추가
  - 자유 입력은 4.4의 영문 패턴을 만족할 때만 칩으로 변환
  - 한글 미확정 입력은 칩으로 추가되지 않음(드롭다운에서 선택해야 함)
...
- 입력 검증 가드 테스트(`web/lib/ticker-input.test.cjs` 또는 동일 파일에 추가):
  - 한글 단독 입력(`애플`, `알파벳A`)은 확정 불가
  - 한영 혼합(`알파벳GOOGL`)은 확정 불가
  - 영문 패턴 만족(`MSTR`, `BRK.B`, `BF-B`)은 확정 가능
  - 패턴 미달(`a`, `MSTR ABC`, `MSTR!`)은 확정 불가
  - 시드 별칭 정확일치(`알파벳A`)는 단일 시드 결과로 자동 확정 → `GOOGL` 반환
...
- `./dev.sh`로 dev 서버 가동
- 세 폼 각각에서:
  - 한글 검색(`알파`, `테슬`, `애플`) → 드롭다운 선택 시 정상 제출
  - 한글 검색 후 **선택 없이 Enter/제출** 시도 → **차단되고 안내 메시지** 노출
  - 영문 검색(`googl`, `Goo`, `aapl`) 결과/선택 동작
  - 시드 미존재 영문 티커(`MSTR`, `BRK.B`) 자유 입력 후 제출 가능
  - 영문이지만 패턴 미달(`MSTR!`, 공백 포함)은 확정/제출 차단
  - 키보드 ↑↓/Enter/Esc 동작
...
| 6 | 자유 입력은 영문 티커 패턴(`^[A-Z][A-Z0-9.\-]{0,15}$`)만 허용 | 시드 누락 영문 종목은 통과시키되, 한글/별칭 미확정 입력이 백엔드까지 흘러가는 실패 경로(분석 실패, 잘못된 보유/스케줄 데이터)를 프론트에서 차단. 서버사이드 ticker 검증 추가는 v1 범위 밖(별도 스펙) |
