...
def default_auth_path() -> Path:
    """Return the default TradingAgents Codex OAuth credential path."""
    env_path = os.environ.get("TRADINGAGENTS_CODEX_OAUTH_AUTH_PATH")
    if env_path:
        return Path(env_path).expanduser()
    return Path.home() / ".tradingagents" / "codex_oauth" / "auth.json"
    ...
    def load(self) -> OAuthCredentials:
        ...
        if not self.auth_path.exists():
            raise NotAuthenticatedError(
                ...
                "tradingagents.llm_clients.codex_oauth login`."
            )
